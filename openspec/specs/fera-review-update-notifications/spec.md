# Fera Review Update Notifications

## Purpose

Persist Fera review snapshots and notify operators about qualifying review updates while preserving existing negative-review notification behavior.
...
- **WHEN** normalized `media` changed between the previous and current snapshots
- **AND** the current snapshot contains no media items that were absent in the previous snapshot
